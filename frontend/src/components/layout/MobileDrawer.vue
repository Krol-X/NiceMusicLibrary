...
const { isSidebarVisible, isMobile } = storeToRefs(uiStore)
const { isAuthenticated } = storeToRefs(authStore)

const showDrawer = computed(() => isMobile.value && isSidebarVisible.value)
...
const mainNavItems: NavItem[] = [
  ...
  { name: 'Библиотека', path: '/library', icon: 'library', requiresAuth: true },
]

const userNavItems: NavItem[] = [
  { name: 'Избранное', path: '/favorites', icon: 'heart', requiresAuth: true },
  { name: 'Недавние', path: '/recent', icon: 'clock', requiresAuth: true },
  { name: 'Mood Chains', path: '/mood-chains', icon: 'chain', requiresAuth: true },
  { name: 'Плейлисты', path: '/playlists', icon: 'playlist', requiresAuth: true },
  { name: 'Статистика', path: '/stats', icon: 'chart', requiresAuth: true },
]
...
const icons: Record<string, string> = {
  ...
  playlist: '<path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z" />',
...
<template>
  <Teleport to="body">
    ...
    <Transition name="slide">
      <aside
        v-if="showDrawer"
        class="fixed left-0 top-0 z-50 flex h-full w-72 flex-col bg-bg-primary shadow-xl"
      >
        <!-- Header -->
        <div class="flex h-16 items-center justify-between border-b border-bg-tertiary px-4">
          <router-link
            to="/"
          ...
          >
            ...
            <span class="text-h3 font-bold text-text-primary">
              NiceMusic
            </span>
          </router-link>
          <button
        ...
        <nav class="flex-1 overflow-y-auto p-4 scrollbar-thin">
          ...
          <div class="mb-6">
            ...
            >
              <button
                ...
                type="button"
                :class="[
                  'flex w-full items-center gap-3 rounded-lg px-3 py-3 transition-colors duration-fast',
                  isActive(item.path)
          ...
          <div
            ...
            class="mb-6"
          >
            <p class="mb-3 px-3 text-caption font-medium uppercase tracking-wider text-text-muted">
              Моя музыка
            </p>
            <template
        ...
        <!-- Bottom Navigation -->
        <div class="border-t border-bg-tertiary p-4">
          <template
